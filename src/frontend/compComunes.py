...
def crear_lista_claves(parent):
    ...
    padx_derechos = [70, 92, 55, 0]
    for i, encabezado in enumerate(encabezados):
        label = ctk.CTkLabel(header_frame, text=encabezado, font=("Segoe UI", 14, "bold"), text_color="#111111")
        label.grid(row=0, column=i, padx=(4,padx_derechos[i]), pady=2, sticky="w")
    ...
    # Procesar cada tipo de clave
    for algoritmo in ["sphincs", "dilithium"]:
        for clave in claves_disponibles.get(algoritmo, []):
                    row_count = agregar_fila_clave(lista_frame, row_count, algoritmo, clave)
...
def agregar_fila_clave(lista_frame, row_count, algoritmo, clave):
    """
    Añade una fila con información de clave al frame scrollable
    """
    # Variable para rastrear si la clave está caducada
    es_caducada = False
    ...
    try:
        ...
        es_caducada = fecha_cad < fecha_actual
        es_futura = fecha_exp > fecha_actual
     ...
     # Mostrar logo o nombre del algoritmo
    if algoritmo in LOGO_IMAGES and LOGO_IMAGES[algoritmo]:
        logo_label = tk.Label(fila_container, image=LOGO_IMAGES[algoritmo], bg="#FFFFFF", cursor="hand2")
        logo_label.grid(row=0, column=0, padx=10, pady=5, sticky="w")
    else:
        alg_nombre = "SPHINCS+" if algoritmo == "sphincs" else "Dilithium"
        ctk.CTkLabel(fila_container, text=alg_nombre, font=("Segoe UI", 12), cursor="hand2").grid(
            row=0, column=0, padx=10, pady=5, sticky="w")
    
    # Título
    ctk.CTkLabel(fila_container, text=clave["titulo"], font=("Segoe UI", 12), cursor="hand2").grid(
        row=0, column=1, padx=10, pady=5, sticky="w")
    
    # Clave pública truncada
    ...
    clave_truncada = clave_publica[:15] + "..." if len(clave_publica) > 15 else clave_publica
    ...
    pk_label.grid(row=0, column=2, padx=10, pady=5, sticky="w")
     ...
     # MODIFICACIÓN: Solo hacer clicable si no está caducada
    if not es_caducada:
        ...
        # También vincular el evento a todos los widgets hijos
        for child in fila_container.winfo_children():
            child.bind("<Button-1>", on_click)
    else:        
        # Para claves caducadas, cambiar el aspecto visual
        fila_container.configure(bg="#F5F5F5")  # Gris muy claro
